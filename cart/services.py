...
@transaction.atomic
def update_quantity(request: HttpRequest, item_id: int, quantity: int) -> Optional[CartItem]:
    """
    Update the requested quantity for a specific line item in the cart.
...
def merge_carts(request: HttpRequest, user: Any) -> Optional[Cart]:
    ...
    Workflow:
    1. Look up guest cart by session key. If none exists, return user cart.
    ...
    4. Delete guest cart upon completion.
    """
    session_key = getattr(request.session, "session_key", None)
    if not session_key:
        return Cart.objects.filter(user=user).first()

    guest_cart = Cart.objects.filter(session_key=session_key).prefetch_related("items__product").first()
    ...
    for guest_item in guest_cart.items.all():
        ...
        user_item = user_cart.items.filter(product=product).first()
        if user_item:
...
def calculate_totals(cart: Optional[Cart]) -> Dict[str, Any]:
    """
    Calculate summary statistics for a shopping cart.
    """
    if not cart:
        return {
            "item_count": 0,
